    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        ...
        {tags.map(tag => (
          <Badge
            key={tag.id}
            className={`${tagColors[tag.color]} text-[10px] font-semibold px-2 py-0.5 rounded`}
          >
            {tag.name}
          </Badge>
        ))}
        {isAICard && (
          <Bot className="w-3.5 h-3.5 text-purple-500 ml-auto" />
        )}
      </div>

      {/* Card Title */}
      <h3 className="text-sm font-medium text-gray-900 mb-3 leading-snug">
        {card.title}
      </h3>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-400">{formatRelativeTime(card.createdAt)}</span>
        ...
        <div className="flex items-center gap-1">
          {assignee && (
            <>
              ...
              <span className="text-gray-500 font-medium">{assignee.name.split(' ')[0]}</span>
              <Avatar className="w-5 h-5">
      ...
      {card.version > 1 && (
        <div className="mt-2 pt-2 border-t border-gray-100">
          <span className="text-[10px] text-gray-400">
            v{card.version} • Last modified by {getUserById(card.lastModifiedBy)?.name || 'Unknown'}
          </span>
        </div>
      )}
    </div>
